...
The simulation is 80 ps total (~40 000 integration steps). On a modern CPU expect roughly 10–20 minutes; on a CUDA GPU it should finish in a couple of minutes. To run faster, set `platform.name: CUDA` in `config.yaml`.
