"""Stage framework: the base classes, the type registry, and discovery.

Kept out of ``__init__`` so the package initializer stays a thin re-export plus
the discovery call. Stage modules import their base/decorator from the package
(``from . import SimulationStage, register_stage``), which re-exports these.
"""
...
def discover_stages(package_name: str, package_path) -> None:
    """Import every stage module in the package so its ``@register_stage`` runs.

    ``_``-prefixed modules (like this one) are skipped, so the framework isn't
    re-imported as if it were a stage.
    """
